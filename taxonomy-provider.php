<?php
/**
 * provider taxonomy archive — the games of a single provider.
 *
 * Mirrors taxonomy-game_category.php: same banner, filter toolbar, AJAX grid,
 * load-more, SEO content block and FAQ. Two differences follow from `provider`
 * being a flat taxonomy attached to games:
 *   - the single-select dropdown lists game categories (not child terms), and
 *   - there is no Provider dropdown; the archive is already one provider, and
 *     the grid stays scoped to it through `data-provider` (see solaire_ajax_load_games).
 */
...
// Hero description comes from the provider's Description field.
$blurb = trim(wp_strip_all_tags(term_description()));

$current_term = get_queried_object();

// Term featured image (ACF image array) overrides the fallback banner.
$featured_image = get_field('so_provider_featured_image', $current_term);
if (is_array($featured_image) && !empty($featured_image['url'])) {
    $banner     = $featured_image['url'];
    $banner_alt = $featured_image['alt'] ?? '';
}

// Category filter = top-level game categories (single-select). Themes filter =
// the "Tags" taxonomy (game-tag), multi-select. There is no Provider dropdown
// here — the archive itself is the provider.
...
// When the provider has no games at all, skip the filters/grid/load-more
// entirely rather than rendering an empty toolbar over a "No games found." box.
$has_games = have_posts();

// H1 = ACF "Provider Main Title" on the term, falling back to the generated
// "Play {Provider} Games in the Philippines" headline.
$main_title = trim((string) get_field('so_provider_main_title', $current_term));
if ($main_title === '') {
    $main_title = sprintf(
        /* translators: %s: game provider name. */
        __('Play %s Games in the Philippines', 'solaire'),
        $page_title
    );
}
...
<main class="mx-auto max-w-shell px-4">
  ...
  <div class="mt-8" data-filter-group data-filter-target="#games-grid">
    <div class="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center">
      ...
      <!-- Game categories (single-select). Always visible. -->
      ...
      <!-- Mobile-only toggle that opens the Themes panel below. -->
      ...
      <div class="solaire-filters-panel" data-filters-panel>
        ...
        <!-- Themes — Tags taxonomy (multi-select) -->
  ...
  <div id="games-grid" data-grid
       ...
       data-parent=""
       data-provider="<?php echo esc_attr(isset($current_term->slug) ? $current_term->slug : ''); ?>"
       data-page="1"
  ...
  <!-- ===================== FAQ ===================== -->
  <?php
  // FAQ items — ACF repeater on the provider term.
  $faq_items = get_field('so_provider_faq_wrapper', $current_term);
  ...
  if ($faq_items && function_exists('solaire_collect_faq_items')) {
      solaire_collect_faq_items(array_map(function ($faq) {
          return [
              'question' => $faq['so_provider_faq_question'] ?? '',
              'answer'   => $faq['so_provider_faq_answer'] ?? '',
          ];
      }, $faq_items));
  }

  // FAQ heading = ACF "Provider FAQ Title" on the term, falling back to the
  // generated "A Quick Guide to {Provider} Games" headline.
  $faq_title = trim((string) get_field('so_provider_faq_title', $current_term));
  if ($faq_title === '') {
      $faq_title = sprintf(
          /* translators: %s: game provider name. */
