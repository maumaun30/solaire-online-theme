...
<section <?php echo get_block_wrapper_attributes(['class' => 'bg-surface py-8 sm:py-16']); ?>>
  <div class="mx-auto max-w-shell px-4">
    ...
    <div class="-mx-2 mt-10 flex flex-wrap justify-center gap-y-8 text-center md:-mx-4">
  <?php foreach ($benefits as $i => $b) :
      ...
      $delay = $i * 100;
  ?>
    <div data-anim <?php echo $delay ? 'data-anim-delay="' . esc_attr($delay) . '"' : ''; ?> class="flex w-1/2 shrink-0 grow-0 flex-col items-center px-2 md:w-1/3 md:px-4">
      ...
      <p class="mt-1.5 max-w-[13rem] text-xs leading-snug text-slatey md:mt-2 md:max-w-xs md:text-sm md:leading-relaxed"><?php echo esc_html($text); ?></p>
    </div>
  <?php endforeach; ?>
</div>
  </div>
