...
<main class="mx-auto max-w-shell px-4 pt-6">
  ...
  <!-- ===================== ABOUT ===================== -->
  <?php
  // Rendered once and reused for both the visible copy and the FAQPage schema,
  // so the two can never describe different text.
  $about_rendered = apply_filters('the_content', get_the_content());

  // Editors write the FAQ as prose inside this copy, so it has to be parsed out.
  if (function_exists('solaire_parse_faq_pairs')) {
      solaire_collect_faq_items(solaire_parse_faq_pairs($about_rendered));
  }

  if (trim(strip_tags($about_rendered)) !== '') : ?>
